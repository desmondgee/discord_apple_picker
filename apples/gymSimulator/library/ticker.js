const Random = require(__basedir + '/shared_library/random');
...
function calculateVisitCount(_user, plot, occupancy) {
...
function buildClimbers(currentTime, numberOfClimbers, user, plot) {
  newClimbers = [];
...
async function updatePlot(user, plot) {
  if (!plot) return;

  let climbers = await Climber.find({ plotId: plot._id }).exec();

  const minutes = (new Date().getTime() - user.lastSync.getTime()) / (60 * 1000);
  let ticks = Math.floor(minutes / minutesPerTick);

  if (ticks <= 0) return;
  if (ticks > maxTicks) ticks = maxTicks;

  let lastTime = new Date(user.lastSync.getTime());

  for (let tick = 1; tick <= ticks; tick++) {
    const time = new Date(user.lastSync.getTime() + tick * minutesPerTick * 60 * 1000);
    ...
    const newVisitCount = calculateVisitCount(user, plot, climbers.length);
    const newClimbers = buildClimbers(time, newVisitCount, user, plot);
